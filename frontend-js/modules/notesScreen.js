...
export default class NotesScreen {
  constructor() {
    ...
    this.internalLinksSideBarWrapper = document.getElementById(
  ...
  getNotesFromLocalStorage() {
    ...
    subjects.forEach((subject) => {
      subjectWiseUnits[subject] = subjectWiseUnits[subject].sort(
        (a, b) => a.unitNo - b.unitNo
      );
    });
  ...
  addInTheSidebar(subjects, subjectWiseUnits) {
    if (subjects.length) {
      subjects.forEach((subject) => {
        this.unitsSideBarWrapper.insertAdjacentHTML(
          "beforeend",
          `
                    <div class="subject">
    ...
    function toggleTheUnits(e) {
      ...
      if (!isSubjectActive) {
        let calculatedHeight =
          window.innerHeight * 0.07 * itsSibling.childElementCount;
    ...
    axios
      ...
      .then((response) => {
        if (response.status === 200)
          this.toggleButtons(
            this.saveNoteForm.lastElementChild,
            "saveNote--saved",
            ["Save", "Saved"]
          );
      })
  ...
  internalLinksHandler() {
    this.internalLinksSideBarWrapper.insertAdjacentHTML(
      "beforeend",
      Array.from(this.subTopics)
        .map((subTopic) => {
          subTopic.parentElement.setAttribute(
            "id",
            encodeURIComponent(subTopic.textContent)
          );
          return `
                    <div class="internalLink">
                        <a href="#${encodeURIComponent(
                          subTopic.textContent
                        )}">${subTopic.textContent}</a>
                    </div>
                `;
        })
        .join("")
