...
export default class UserQuery {
    constructor() {
        ...
        this.searchTerm = document.querySelector("#userQueryScreen input[name=searchTerm]");
        this.basedOn = document.querySelector("#userQueryScreen select[name=basedOn]");
    ...
    sendUserQuery(searchTermValue, basedOnValue) {
        axios.post("/admin/userQuery", { searchTerm: searchTermValue, basedOn: basedOnValue })
            .then((response) => {
                this.clearUserCard();
                if (Object.keys(response.data).length)
                    this.createUserCard(response.data);
                else
                    this.userCardContainer.insertAdjacentHTML("beforeend", `<h2>No users found</h2>`);
            })
            .catch((error) => {
                console.log(error);
            })
    ...
    createUserCard(users) {
        ...
        for (let i = 0; i < newUsersArray.length; i++) {
            ...
            userCard.innerHTML = `
                ...
                <p>username: ${newUsersArray[i].username} </p>
                <p>email: ${newUsersArray[i].email} </p>
                <p>faculty: ${newUsersArray[i].faculty} </p>
                <p>semester: ${newUsersArray[i].semester} </p>
                <p>joinedOn: ${newUsersArray[i].joinedOn.date}/${newUsersArray[i].joinedOn.month}/${newUsersArray[i].joinedOn.year}</p>
                <ul>
                    ${newUsersArray[i].roles.map(role => `<li>${role}</li>`).join("")}
                </ul>
            `
    ...
    setActions(user, userCard) {
        ...
        //approval
        if (user.isApproved)
            actions.push({ userId: user._id, action: "/admin/disapprove-single", value: "Disapprove" });
        else actions.push({ userId: user._id, action: "/admin/approve-single", value: "Approve" });

        //subscription
        if (user.isSubscriptionExpired)
            actions.push({ userId: user._id, action: "/admin/renewSubscription-single", value: "Renew Subscription" });
        else actions.push({ userId: user._id, action: "/admin/expireSubscription-single", value: "Expire Subscription" });
        ...
        if (user.roles.length > 1) {
            user.roles.forEach((role) => {
                switch (role) {
                    case "contributor":
                        actions.push({ userId: user._id, action: "/admin/removeAsContributor", value: "Remove as Contributor" });
                }
            })
        }
        else {
            actions.push({ userId: user._id, action: "/admin/makeContributor", value: "Make Contributor" });
        }
    ...
    doTheAction(action) {
        ...
        axios.post(action.action, { userId: userId })
            .then((response) => {
